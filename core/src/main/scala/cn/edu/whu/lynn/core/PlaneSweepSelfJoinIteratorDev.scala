 ...
 */
class PlaneSweepSelfJoinIteratorDev[T <: IFeature]
(var r: Array[T], dupAvoidanceMBR: EnvelopeNDLite, metrics: MetricsAccumulator = null, keepLeft: Boolean = false)
  ...
  var j: Int = i+1
  var jj: Int = j
  var foundNoMatch: Boolean = true // Initially set to true to handle the case where no match is found in the first iteration
  ...
  private def seekToNextOutput(): Boolean = {
    ...
    while (i < r.length) {
      jj = j
      while (jj < r.length && xmin(j) <= xmax(i)) {
        if (numMBRTests != null) numMBRTests.add(1)
        if (rectangleOverlaps(i, jj) && referencePointTest(i, jj)) {
      ...
      if (foundNoMatch && keepLeft) {
        // If no match was found for the current left record, emit it as a self-join
        count += 1
        return true
      }
      i += 1
  ...
  override def next(): (T, T) = {
    if (foundNoMatch && keepLeft) {
      // If no match was found for the current left record, emit it as a self-join
      val matchedPair = (r(i), null.asInstanceOf[T])
      foundNoMatch = false
      i += 1
      j = i + 1
      seekToNextOutput()
