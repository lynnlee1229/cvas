...
import cn.edu.whu.lynn.geolite.{EmptyGeometry, GeometryHelper}
...
trait IFeature extends Serializable with Row {
  ...
  // TODO 检查这个方法的实现是否正确，目前是将PointND强制转为Point
//  def getGeometry: Geometry = {
//    if (iGeom == -1) {
//      EmptyGeometry.instance
//    } else {
//      val geom = get(iGeom)
//      if (geom.isInstanceOf[PointND]) {
//        // 如果是 PointND 类型，则进行类型转换为 Point
//        val pointND = geom.asInstanceOf[PointND]
//        val coordinates = new Coordinate(pointND.getCoordinate())
//        new GeometryFactory().createPoint(coordinates)
//      } else {
//        // 如果不是 PointND 类型，直接进行类型转换
