...
import cn.edu.whu.lynn.core.index.quadSplitedGeom.{QuadSplitedLineString, QuadSplitedMultiLineString, QuadSplitedMultiPolygon, QuadSplitedPolygon}
import cn.edu.whu.lynn.geolite.{Feature, GeometryReader, GeometryType}
import SpatialDataTypes.SpatialRDD
import org.apache.spark.cvas.sql.GeometryDataType
 ...
 */
class GeometryAdaptiveSplitter(geometry: Geometry, threshold: Int) extends Iterator[Geometry] {
  val toBreak: mutable.ArrayBuffer[Geometry] = new mutable.ArrayBuffer[Geometry]()
  toBreak.append(geometry)

  override def hasNext: Boolean = {
    return toBreak.nonEmpty
  }
  ...
  override def next(): Geometry = {
    ...
    while (g != null && (g.getNumPoints > threshold || g.getNumGeometries > 1)) {
      if (g.getNumGeometries > 1) {
        ...
        try {
          toBreak.append(g.intersection(g.getFactory.toGeometry(new Envelope(e.getMinX, centerX, e.getMinY, centerY))))
        }
        catch {
          case _: Exception =>
        }
        try {
          toBreak.append(g.intersection(g.getFactory.toGeometry(new Envelope(centerX, e.getMaxX, e.getMinY, centerY))))
        }
        catch {
          case _: Exception =>
        }
        try {
          toBreak.append(g.intersection(g.getFactory.toGeometry(new Envelope(e.getMinX, centerX, centerY, e.getMaxY))))
        }
        catch {
          case _: Exception =>
        }
        try {
          g = g.intersection(g.getFactory.toGeometry(new Envelope(centerX, e.getMaxX, centerY, e.getMaxY)))
        }
        catch {
          case _: Exception => g = if (toBreak.isEmpty) null else toBreak.remove(toBreak.length - 1)
        }
      }
    }
    g
...
object GeometryAdaptiveSplitter {
  /**
   ...
   * @return
   */
  def adapSplitRDD(spatialRDD: SpatialRDD, threshold: Int, keepBothGeometries: Boolean, adaptiveVal: Int = 256): SpatialRDD = {
    spatialRDD.mapPartitions(features => {
      features.flatMap(feature => {
        val thisThreshold = math.max(feature.getGeometry.getNumPoints / adaptiveVal, threshold)
        val smallGeometries: Iterator[Geometry] = new GeometryAdaptiveSplitter(feature.getGeometry, thisThreshold)
  ...
  def adapSplitRDDToIndexedGeom(spatialRDD: SpatialRDD, threshold: Int, keepBothGeometries: Boolean, adaptiveVal: Int = 256): SpatialRDD = {
    spatialRDD.flatMap(
      feature => {
        val thisThreshold = math.max(feature.getGeometry.getNumPoints / adaptiveVal, threshold)
        if (feature.getGeometry.getNumPoints < thisThreshold) {
          Iterator(feature)
        } else {
          val smallGeometries: Iterator[Geometry] = new GeometryAdaptiveSplitter(feature.getGeometry, thisThreshold)
          feature.getGeometry.getGeometryType match {
            case GeometryType.POLYGON.typename => {
              val simplifiedGeometry = QuadSplitedPolygon.fromGeom(feature.getGeometry.asInstanceOf[Polygon], smallGeometries.toList.asJava)
              List(Feature.create(feature, simplifiedGeometry))
            }
            case GeometryType.MULTIPOLYGON.typename => {
              val simplifiedGeometry = QuadSplitedMultiPolygon.fromGeom(feature.getGeometry.asInstanceOf[MultiPolygon], smallGeometries.toList.asJava)
              List(Feature.create(feature, simplifiedGeometry))
            }
            case GeometryType.LINESTRING.typename => {
              val simplifiedGeometry = QuadSplitedLineString.fromGeom(feature.getGeometry.asInstanceOf[LineString], smallGeometries.toList.asJava)
              List(Feature.create(feature, simplifiedGeometry))
            }
            case GeometryType.MULTILINESTRING.typename => {
              val simplifiedGeometry = QuadSplitedMultiLineString.fromGeom(feature.getGeometry.asInstanceOf[MultiLineString], smallGeometries.toList.asJava)
              List(Feature.create(feature, simplifiedGeometry))
            }
            case _ => {
              if (keepBothGeometries) {
  ...
  def adapSplitRDDToIndexedGeomBak(spatialRDD: SpatialRDD, threshold: Int, keepBothGeometries: Boolean, adaptiveVal: Int = 256): SpatialRDD = {
    spatialRDD.mapPartitions(features => {
      features.flatMap(feature => {
        val thisThreshold = math.max(feature.getGeometry.getNumPoints / adaptiveVal, threshold)
        val smallGeometries: Iterator[Geometry] = if (feature.getGeometry.getNumPoints > thisThreshold) {
          new GeometryAdaptiveSplitter(feature.getGeometry, thisThreshold)
        } else {
          Iterator(feature.getGeometry)
        }
        if (feature.getGeometry.getNumPoints > thisThreshold) {
          feature.getGeometry.getGeometryType match {
            case GeometryType.POLYGON.typename => {
