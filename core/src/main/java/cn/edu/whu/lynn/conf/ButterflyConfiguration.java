package cn.edu.whu.lynn.conf;

import cn.edu.whu.lynn.exception.ButterflyException;
import cn.edu.whu.lynn.exception.CommonErrorCode;
import cn.edu.whu.lynn.exception.ErrorCode;
...
public class ButterflyConfiguration implements Serializable{
     ...
     * 初始化空白的Configuration
     */
    public static ButterflyConfiguration newDefault() {
        return ButterflyConfiguration.from("{}");
    }

    /**
     * 从JSON字符串加载Configuration
     */
    public static ButterflyConfiguration from(String json) {
        json = StrUtils.replaceVariable(json);
        checkJSON(json);

        try {
            return new ButterflyConfiguration(json);
        } catch (Exception e) {
            throw ButterflyException.asDataXException(CommonErrorCode.CONFIG_ERROR,
                    e);
        }
     ...
     */
    public static ButterflyConfiguration from(File file) {
        try {
            return ButterflyConfiguration.from(IOUtils
                    .toString(new FileInputStream(file)));
        } catch (FileNotFoundException e) {
            throw ButterflyException.asDataXException(CommonErrorCode.CONFIG_ERROR,
                    String.format("配置信息错误，您提供的配置文件[%s]不存在. 请检查您的配置文件.", file.getAbsolutePath()));
        } catch (IOException e) {
            throw ButterflyException.asDataXException(
     ...
     */
    public static ButterflyConfiguration from(InputStream is) {
        try {
            return ButterflyConfiguration.from(IOUtils.toString(is));
        } catch (IOException e) {
            throw ButterflyException.asDataXException(CommonErrorCode.CONFIG_ERROR,
     ...
     */
    public static ButterflyConfiguration from(final Map<String, Object> object) {
        return ButterflyConfiguration.from(ButterflyConfiguration.toJSONString(object));
    }

    /**
     * 从List对象加载Configuration
     */
    public static ButterflyConfiguration from(final List<Object> object) {
        return ButterflyConfiguration.from(ButterflyConfiguration.toJSONString(object));
    }
    ...
    public String getNecessaryValue(String key, ErrorCode errorCode) {
        ...
        if (StringUtils.isBlank(value)) {
            throw ButterflyException.asDataXException(errorCode,
     ...
     */
    public ButterflyConfiguration getConfiguration(final String path) {
     ...
     */
    public List<ButterflyConfiguration> getListConfiguration(final String path) {
        List<Object> lists = getList(path);
        if (lists == null) {
            return null;
        }

        List<ButterflyConfiguration> result = new ArrayList<ButterflyConfiguration>();
        for (final Object object : lists) {
            result.add(ButterflyConfiguration.from(ButterflyConfiguration.toJSONString(object)));
        }
        return result;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public Map<String, ButterflyConfiguration> getMapConfiguration(final String path) {
        Map<String, Object> map = this.get(path, Map.class);
        if (null == map) {
            return null;
        }

        Map<String, ButterflyConfiguration> result = new HashMap<String, ButterflyConfiguration>();
        for (final String key : map.keySet()) {
            result.put(key, ButterflyConfiguration.from(ButterflyConfiguration.toJSONString(map
                    .get(key))));
        }
     ...
     * @return 返回合并后对象
     */
    public ButterflyConfiguration merge(final ButterflyConfiguration another,
    ...
    public String toJSON() {
        return ButterflyConfiguration.toJSONString(this.getInternal());
    }
     ...
     */
    public ButterflyConfiguration clone() {
        ButterflyConfiguration config = ButterflyConfiguration
                .from(ButterflyConfiguration.toJSONString(this.getInternal()));
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw ButterflyException.asDataXException(CommonErrorCode.RUNTIME_ERROR,
    ...
    private Object extractConfiguration(final Object object) {
        if (object instanceof ButterflyConfiguration) {
    ...
    private Object extractFromConfiguration(final Object object) {
        if (object instanceof ButterflyConfiguration) {
            return ((ButterflyConfiguration) object).getInternal();
        }
    ...
    private ButterflyConfiguration(final String json) {
        try {
    ...
    public boolean getBoolean(String path, boolean defaultValue) {
        return getBool(path, defaultValue);
    }
}