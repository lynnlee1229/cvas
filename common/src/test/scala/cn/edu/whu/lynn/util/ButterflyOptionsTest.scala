...
class ButterflyOptionsTest extends FunSuite with ScalaSparkTest {
  test("retain index") {
    val opts = new ButterflyOptions().set("key1[1]", "val1")
      .set("key1[2]", "val2")
      .set("key3", "val3")
      .set("key4[2]", "val4")
  ...
  test("Load defaults") {
    ...
    val opts2 = new ButterflyOptions()
    assert(opts2.size == 1)
    assert(opts2("testkey") == "testvalue")
  }

  test("Convert from a map") {
    def opts: ButterflyOptions = Map("key1" -> "value1", "key2" -> "value2")
