...
import ButterflyOptions.{defaultHadoopConf, defaultSparkConf}
 ...
 */
class ButterflyOptions(loadDefaults: Boolean = true)
  extends scala.collection.mutable.HashMap[String, String] with Serializable {

  if (loadDefaults)
    this.mergeWith(ButterflyOptions.defaultOptions)

  /**
   * Copy constructor
   * @param bo the other BeastOptions to copy from
   */
  def this(bo: ButterflyOptions) {
    this(false)
    for (entry <- bo)
   ...
   * @return
   */
  def set(key: String, value: Any): ButterflyOptions = {
   ...
   * @return
   */
  def setInt(key: String, value: Int): ButterflyOptions = this.set(key, value.toString)
   ...
   * @return
   */
  def setLong(key: String, value: Long): ButterflyOptions = this.set(key, value.toString)
   ...
   * @return
   */
  def setBoolean(key: String, value: Boolean): ButterflyOptions = this.set(key, value.toString)
   ...
   */
  def retainIndex(index: Int): ButterflyOptions = {
    val suffix = s"[$index]"
    val newOpts = new ButterflyOptions(false)
    this.foreach(kv => {
      if (kv._1.indexOf('[') == -1)
   ...
   */
  def loadFromTextFile(fs: FileSystem, path: Path): ButterflyOptions = {
    val in = fs.open(path)
    try {
      loadFromTextFile(in)
    } finally in.close()
   ...
   */
  def loadFromTextFile(in: InputStream): ButterflyOptions = {
  ...
  def mergeWith(opts: ButterflyOptions): ButterflyOptions = {
    for ((k, v) <- opts)
      this.set(k, v)
    this
  }
...
object ButterflyOptions {
  implicit def fromPairs(map: Iterable[(String, Any)]): ButterflyOptions = {
    val bo = new ButterflyOptions()
    for (entry <- map)
      bo.put(entry._1, entry._2.toString)
    bo
  }

  implicit def fromPair(entry: (String, Any)): ButterflyOptions = new ButterflyOptions().set(entry._1, entry._2.toString)

  implicit def fromMap(map: java.util.Map[String, String]): ButterflyOptions = {
    val bo = new ButterflyOptions()
   ...
   * @return
   */
  def fromStrings(args: Iterable[String]): ButterflyOptions = {
    val options = new ButterflyOptions()
    val optionName = "((\\w+)(\\[\\d+\\])?)"
   ...
   * @return
   */
  def fromStringArray(args: Array[String]): ButterflyOptions = fromStrings(args.toIterable)

  implicit def fromSparkConf(sparkConf: SparkConf): ButterflyOptions = fromPairs(sparkConf.getAll)

  lazy val defaultOptions: ButterflyOptions = {
    val opts = new ButterflyOptions(false)
    ...
    val localfile: File = new File("cvas.properties")
