...
class JavaSpatialSparkContext(val _sc: SparkContext) extends JavaSparkContext(_sc) {
   ...
   */
  def spatialFile(filename: String, iformat: String, opts: ButterflyOptions): JavaSpatialRDD =
  ...
  def spatialFile(filename: String, iformat: String): JavaSpatialRDD =
    spatialFile(filename, iformat, new ButterflyOptions())
   ...
   */
  def spatialFile(filename: String, opts: ButterflyOptions): JavaSpatialRDD =
    spatialFile(filename, opts.getString(SpatialFileRDD.InputFormat), new ButterflyOptions())
