...
import cn.edu.whu.lynn.core.{SJAlgorithm, SJPredicate}
import cn.edu.whu.lynn.generator.{ParcelDistribution, UniformDistribution}
import cn.edu.whu.lynn.geolite._
...
class SpatialJoinScalaTest extends FunSuite with ScalaSparkTest {
  test("SpatialJoinDupAvoidance") {
    ...
    val results = SpatialJoin.spatialJoinIntersectsPlaneSweepFeatures(r.toArray, s.toArray,
      new EnvelopeNDLite(2, 2.0, 0.0, 5.0, 3.0), SJPredicate.Intersects, null)
  ...
  test("Distributed Join on loaded indexes") {
    ...
    val joinResults = dataset1.spatialJoin(dataset2, SJPredicate.Intersects, SJAlgorithm.DJ)
  ...
  test("Distributed Join on memory-partitioned data") {
    ...
    val joinResults = partitioned1.spatialJoin(partitioned2, SJPredicate.Intersects)
  ...
  test("PBSM with points") {
    ...
    val result = SpatialJoin.spatialJoinPBSM(dataset1, dataset2, SJPredicate.MBRIntersects)
  ...
  test("Close-by but not overlapping records") {
    ...
    val results = SpatialJoin.spatialJoinBNLJ(r1, r2, joinPredicate = SJPredicate.MBRIntersects)
      ...
      for (i <- 0 until 10) {
        for (distribution <- distributions) {
          ...
          val secondDataset: SpatialRDD = sparkContext.generateSpatialData(UniformDistribution, 100,
            ...
            .zipWithUniqueId()
            .map(addRecordID)
          val bnljResults = firstDataset.spatialJoin(secondDataset, method = SJAlgorithm.BNLJ)
          // We use the BNLJ as the baseline (groundtruth)
          val bnljResultSize = bnljResults.count()
          // Check PBSM result
          val pbsmResults = firstDataset.spatialJoin(secondDataset, method = SJAlgorithm.PBSM,
          ...
          val djResults = SpatialJoin.spatialJoinDJ(firstIndex, secondIndex, SJPredicate.MBRIntersects)
          assertResult(true, s"Error in DJ in iteration #$i")(resultsEqual(bnljResults, bnljResultSize, djResults))

          // Check REPJ results
          val repjResults = SpatialJoin.spatialJoinRepJ(firstIndex, secondIndex, SJPredicate.MBRIntersects)
    ...
    if (shouldRunStressTest()) {
      ...
      for (i <- 0 until 10) {
        ...
        val secondDataset: SpatialRDD = sparkContext.generateSpatialData(UniformDistribution, 100,
          ...
          .map(addRecordID)
        val bnljResults = firstDataset.spatialJoin(secondDataset, method = SJAlgorithm.BNLJ,
          joinPredicate = SJPredicate.MBRIntersects)
        // We use the BNLJ as the baseline (groundtruth)
        val bnljResultSize = bnljResults.count()
        // Check PBSM result with non-indexed data
        val pbsmResults = SpatialJoin.spatialJoinPBSM(firstDataset, secondDataset, SJPredicate.MBRIntersects,
