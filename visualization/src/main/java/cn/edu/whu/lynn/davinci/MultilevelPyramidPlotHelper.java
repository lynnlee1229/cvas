...
import cn.edu.whu.lynn.io.CSVEnvelopeEncoder;
...
import cn.edu.whu.lynn.util.MathUtil;
...
public class MultilevelPyramidPlotHelper {
  ...
  public static boolean plotTile(FileSystem fs, Path vizPath, long tileID, OutputStream out,
                                 LongWritable clientTimestamp, ButterflyOptions opts)
      throws IOException {
    ...
    if (opts == null)
      opts = new ButterflyOptions(fs.getConf());
    if (fs.exists(vizMetadataPath))
      opts = opts.mergeWith(new ButterflyOptions().loadFromTextFile(fs, vizMetadataPath));
    ...
    do {
      ...
      if (fs.exists(imgFilePath)) {
        ...
        if (fs.exists(dataFilePath)) {
          ...
          Plotter plotter = Plotter.createAndConfigurePlotter(new ButterflyOptions(opts));
   ...
   */
  public static void writeVisualizationAddOnFiles(FileSystem fs, Path vizPath, ButterflyOptions opts) throws IOException {
    int tileWidth = opts.getInt(SingleLevelPlotHelper.ImageWidth, 256);
    int tileHeight = opts.getInt(SingleLevelPlotHelper.ImageHeight, 256);
