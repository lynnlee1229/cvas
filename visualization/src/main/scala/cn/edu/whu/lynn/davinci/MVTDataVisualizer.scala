...
import cn.edu.whu.lynn.geolite.{EnvelopeNDLite, Feature, GeometryReader, IFeature}
import cn.edu.whu.lynn.synopses.{AbstractHistogram, GeometryToPoints, HistogramOP, Prefix2DHistogram}
import cn.edu.whu.lynn.util.{FileUtil, OperationMetadata, OperationParam, Parallel2, ZipUtil}
import cn.edu.whu.lynn.core.CGOperationsMixin._
import cn.edu.whu.lynn.core.{Reprojector, SpatialDataTypes}
import cn.edu.whu.lynn.core.SpatialDataTypes.SpatialRDD
import cn.edu.whu.lynn.davinci.{MultilevelPyramidPlotHelper, PyramidPartitioner, SubPyramid, TileIndex, VectorTile}
import cn.edu.whu.lynn.indexing.{GridPartitioner, IndexHelper}
import cn.edu.whu.lynn.io.{SpatialFileRDD, SpatialOutputFormat, SpatialWriter}
import cn.edu.whu.lynn.io.ReadWriteMixin._
import cn.edu.whu.lynn.io.SpatialFileRDD.SpatialFilePartition
import MultilevelPlot.MasterTileFileName
...
object MVTDataVisualizer extends CLIOperation with Logging {
  ...
  def plotSingleTileParallel(features: SpatialDataTypes.SpatialRDD, resolution: Int,
                             tileID: Long, buffer: Int = 0, opts: ButterflyOptions = new ButterflyOptions()): VectorTile.Tile = {
  ...
  def plotAllTiles(features: SpatialDataTypes.JavaSpatialRDD, minLevel: Int, maxLevel: Int, resolution: Int,
                   buffer: Int, opts: ButterflyOptions): JavaPairRDD[java.lang.Long, IntermediateVectorTile] =
  ...
  def plotAllTiles(features: SpatialDataTypes.SpatialRDD, levels: Range, resolution: Int,
                   buffer: Int = 0, opts: ButterflyOptions = new ButterflyOptions()): RDD[(Long, IntermediateVectorTile)] = {
    val verticalFlip = new AffineTransform2D(AffineTransform.getScaleInstance(1, -1))
  ...
  def plotSingleTileLocal(fs: FileSystem, vizPath: Path, tileID: Long, out: OutputStream, clientTimestamp: LongWritable,
                          resolution: Int, buffer: Int = 0, opts: ButterflyOptions = new ButterflyOptions()): Unit = {
   ...
   */
  def saveTiles(tiles: JavaPairRDD[java.lang.Long, IntermediateVectorTile], outPath: String, opts: ButterflyOptions): Unit =
   ...
   * @param outPath the path to save the tiles to
   */
  def saveTiles(tiles: RDD[(Long, IntermediateVectorTile)], outPath: String, opts: ButterflyOptions): Unit = {
    val conf: ButterflyOptions = new ButterflyOptions(opts.loadIntoHadoopConf(tiles.sparkContext.hadoopConfiguration))
    tiles.foreach({
      case (id, tile) =>
   ...
   */
  def saveTilesCompact(tiles: JavaPairRDD[java.lang.Long, IntermediateVectorTile], outPath: String, _opts: ButterflyOptions): Unit =
   ...
   * @param _opts additional options that were used for visualization.
   */
  def saveTilesCompact(tiles: RDD[(Long, IntermediateVectorTile)], outPath: String, _opts: ButterflyOptions): Unit = {
    val opts = new ButterflyOptions(tiles.context.hadoopConfiguration).mergeWith(_opts)
   ...
   * @return
   */
  def getIndexHTMLFile(opts: ButterflyOptions): String = {
