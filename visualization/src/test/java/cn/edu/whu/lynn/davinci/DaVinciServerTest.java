...
import cn.edu.whu.lynn.util.BeastServer;
...
public class DaVinciServerTest extends JavaSparkTest {
  ...
  public void testSVGTile() throws Exception {
    ...
    try {
      ...
      ButterflyOptions opts2 = new ButterflyOptions(false).set("port", port);
      server.setup(opts2);
  ...
  public void testStaticSVGTile() throws Exception {
    ...
    try {
      ...
      // Start the server
      int port = new Random().nextInt(10000) + 10000;
      ButterflyOptions opts2 = new ButterflyOptions(false)
          .set("port", port)
  ...
  public void testDownloadAsKMZFile() throws Exception {
    ...
    try {
      ...
      // Start the server
      int port = new Random().nextInt(10000) + 10000;
      ButterflyOptions opts = new ButterflyOptions(false).set("port", port);
      server.setup(opts);
  ...
  public void testDownloadAsShapefile() throws Exception {
    ...
    try {
      ...
      JavaRDD<IFeature> features = SpatialReader.readInput(javaSparkContext(), new ButterflyOptions(),
