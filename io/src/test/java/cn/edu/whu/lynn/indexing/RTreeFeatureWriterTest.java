...
import cn.edu.whu.lynn.util.CounterOutputStream;
...
public class RTreeFeatureWriterTest extends JavaSpatialSparkTest {
  ...
  public void testWriteFeaturesWithDifferentSchema() throws IOException {
    ...
    try {
      reader.initialize(new FileSplit(rtreePath, 0, fileLength, null), new ButterflyOptions());
      int resultCount = 0;
  ...
  public void testWriteMapAttributes() throws IOException {
    ...
    // Now, read the data back
    RTreeFeatureReader reader = new RTreeFeatureReader();
    reader.initialize(new FileSplit(rtreePath, 0, rtreePath.getFileSystem(conf).getFileStatus(rtreePath).getLen(), null), new ButterflyOptions(conf));
  ...
  public void testWriteMultipleRTrees() throws IOException {
    ...
    try (RTreeFeatureReader reader = new RTreeFeatureReader()) {
      reader.initialize(new FileSplit(rtreePath, 0, fileLength, null), new ButterflyOptions(conf));
      int recordCount = 0;
      while (reader.nextKeyValue()) {
        recordCount++;
      }
