...
import cn.edu.whu.lynn.generator.{RandomSpatialRDD, UniformDistribution}
import cn.edu.whu.lynn.indexing.{GridPartitioner, IndexHelper}
import cn.edu.whu.lynn.io.{CSVFeatureReader, SpatialFileRDD}
import cn.edu.whu.lynn.io.shapefile.ShapefileFeatureReader
import cn.edu.whu.lynn.synopses.Summary
import cn.edu.whu.lynn.io.SpatialFileRDD.FilePartition
...
class SpatialFileRDDTest extends FunSuite with ScalaSparkTest {
  ...
  test("Add dependent classes with one input format") {
    ...
    val opts: ButterflyOptions = SpatialFileRDD.InputFormat -> "wkt"
  ...
  test("local read") {
    val inputFile = makeFileCopy("/allfeatures.geojson")
    val opts: ButterflyOptions = SpatialFileRDD.InputFormat -> "geojson"
