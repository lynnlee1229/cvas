 ...
 */
package cn.edu.whu.lynn.io.shapefilev2

import cn.edu.whu.lynn.geolite.GeometryHelper
import cn.edu.whu.lynn.io.shapefilev2
import cn.edu.whu.lynn.test.CvasSpatialTest
...
class ShapefileGeometryReaderTest extends FunSuite with CvasSpatialTest {
  test("Read header") {
  ...
  test("Read empty geometries") {
    ...
    for (expected <- geometries) {
      ...
      val writer = new shapefilev2.ShapefileGeometryWriter(shpfile)
