...
import cn.edu.whu.lynn.util.FileUtil;
import cn.edu.whu.lynn.util.OperationHelper;
 ...
 * from a {@link ButterflyOptions} rather than {@link org.apache.hadoop.mapreduce.TaskAttemptContext}.
...
public abstract class FeatureReader implements Iterable<IFeature>, Iterator<IFeature>, Closeable {
   ...
   */
  public void initialize(InputSplit split, ButterflyOptions conf) throws IOException, InterruptedException {
    isClosed = false;
  }

  public void initialize(SpatialFileRDD.FilePartition partition, ButterflyOptions opts) throws IOException, InterruptedException {
  ...
  public static Map<String, Class<? extends FeatureReader>> getFeatureReaders() {
    ...
    synchronized (FeatureReader.class) {
      if (featureReaders == null) {
        ...
        List<String> readers = OperationHelper.readConfigurationXML("cvas.xml").get("Readers");
        if (readers != null) {
  ...
  public ButterflyOptions autoDetect(ButterflyOptions conf, String input) {
    ...
    if (detected)
      return new ButterflyOptions(false).set(SpatialFileRDD.InputFormat(), metadata.shortName());
    return null;
  }
