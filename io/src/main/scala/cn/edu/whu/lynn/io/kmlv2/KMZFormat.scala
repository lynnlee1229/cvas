 ...
 */
package cn.edu.whu.lynn.io.kmlv2
...
class KMZFormat extends FileFormat with DataSourceRegister {
  ...
  override def prepareWrite(sparkSession: SparkSession, job: Job, options: Map[String, String],
    ...
    override def getFileExtension(context: TaskAttemptContext): String = ".kmz"
    ...
    override def newInstance(filepath: String, dataSchema: StructType, context: TaskAttemptContext): OutputWriter = {
      ...
      val zipFile = new ZipOutputStream(fileSystem.create(outPath))
      zipFile.putNextEntry(new ZipEntry("index.kml"))
      val kmlWriter = new KMLWriter(zipFile, dataSchema)
      new OutputWriter {
