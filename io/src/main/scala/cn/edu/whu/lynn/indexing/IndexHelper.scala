...
import cn.edu.whu.lynn.core.SpatialDataTypes.{JavaPartitionedSpatialRDD, JavaSpatialRDD, PartitionedSpatialRDD, SpatialRDD}
import cn.edu.whu.lynn.core.SpatialPartitioner
import cn.edu.whu.lynn.geolite.{EnvelopeNDLite, GeometryHelper, IFeature}
import cn.edu.whu.lynn.io.{FeatureWriter, SpatialWriter}
import cn.edu.whu.lynn.synopses._
import cn.edu.whu.lynn.util.{IntArray, OperationHelper, OperationParam}
...
object IndexHelper extends Logging {
  ...
  /**The number of partitions is adjusted so that each partition has a number of points*/
  case object PointNum extends PartitionCriterion
...
- Count(c): Create n partitions such that each partition contains around c records""",
  ...
  def computeNumberOfPartitions(numPartitions: NumPartitions, summary: Summary): Int = numPartitions.pc match {
    ...
    case PointNum => Math.ceil(summary.numPoints.toDouble / numPartitions.value).toInt
   ...
   */
  def summarizeDataset(features: SpatialRDD, partitionerClass: Class[_ <: SpatialPartitioner],
   ...
   */
  def runDuplicateAvoidance(features: SpatialRDD): SpatialRDD = {
  ...
  def partitionFeatures(features: SpatialRDD, partitionerClass: Class[_ <: SpatialPartitioner],
                        sizeFunction: IFeature=>Int, opts: ButterflyOptions): PartitionedSpatialRDD = {
  ...
  def partitionFeatures2(features: SpatialRDD, partitionerClass: Class[_ <: SpatialPartitioner],
                        sizeFunction: IFeature=>Int, opts: ButterflyOptions): SpatialRDD = {
    val pInfo = parsePartitionCriterion(opts.getString(IndexHelper.PartitionCriterionThreshold, "Size(128m)"))
    val spatialPartitioner = createPartitioner(features, partitionerClass, pInfo, sizeFunction, opts)
  ...
  def partitionFeatures(features: JavaSpatialRDD, partitionerClass: Class[_ <: SpatialPartitioner],
                        sizeFunction: org.apache.spark.api.java.function.Function[IFeature, Int], opts: ButterflyOptions)
  ...
  def partitionFeatures2(features: JavaSpatialRDD, partitionerClass: Class[_ <: SpatialPartitioner],
                        sizeFunction: org.apache.spark.api.java.function.Function[IFeature, Int],
                         opts: ButterflyOptions) : JavaSpatialRDD =
  ...
  lazy val partitioners: Map[String, Class[_ <: SpatialPartitioner]] = {
    ...
    val partitionerClasses: java.util.List[String] = OperationHelper.readConfigurationXML("cvas.xml").get("SpatialPartitioners")
  ...
  def saveIndex(partitionedFeatures: JavaPartitionedSpatialRDD, path: String, opts: ButterflyOptions): Unit =
  ...
  /**
    ...
    */
  @deprecated("Use saveIndex2", "0.9.2")
  def saveIndex(partitionedFeatures: PartitionedSpatialRDD, path: String, opts: ButterflyOptions): Unit =
   ...
   */
  def saveIndex2J(partitionFeatures: JavaSpatialRDD, path: String, opts: ButterflyOptions): Unit =
   ...
   */
  def saveIndex2(partitionFeatures: SpatialRDD, path: String, opts: ButterflyOptions): Unit = {
