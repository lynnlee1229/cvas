...
import cn.edu.whu.lynn.util.{FileUtil, IConfigurable, OperationException, OperationParam}
import cn.edu.whu.lynn.core.SpatialDataTypes._
import cn.edu.whu.lynn.io.{CSVFeatureReader, CSVFeatureWriter, FeatureReader, FeatureWriter, MultipartFeatureWriter}
...
object SpatialWriter extends IConfigurable with Logging {
  ...
  override def addDependentClasses(opts: ButterflyOptions, parameterClasses: util.Stack[Class[_]]): Unit = {
    ...
    if (oFormat == "*auto*") {
      val detected: Tuple2[Class[_ <: FeatureReader], ButterflyOptions] = SpatialFileRDD.autodetectInputFormat(null, new ButterflyOptions)
      if (detected == null) throw new OperationException("Failed to auto-detect input format")
      val detectedOptions: ButterflyOptions = detected._2
  ...
  /**Java shortcut*/
  def saveFeaturesJ(features: JavaSpatialRDD,  oFormat: String, outPath: String, opts: ButterflyOptions): Unit =
  ...
  /**
    ...
    */
  def saveFeatures(features: SpatialRDD,  oFormat: String, outPath: String, opts: ButterflyOptions): Unit = {
    val _opts: ButterflyOptions = new ButterflyOptions(features.sparkContext.hadoopConfiguration).mergeWith(opts)
