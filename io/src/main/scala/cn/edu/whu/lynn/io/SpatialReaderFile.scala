...
import cn.edu.whu.lynn.io.{FeatureReader, SpatialFileSplit, SpatialReaderMetadata}
...
class SpatialFilePartitionReaderFactory[FR <: FeatureReader](val schema: StructType,
  ...
  val hadoopConf: ButterflyOptions =
    new ButterflyOptions(SparkSession.active.sessionState.newHadoopConfWithOptions(options.asScala.toMap))
